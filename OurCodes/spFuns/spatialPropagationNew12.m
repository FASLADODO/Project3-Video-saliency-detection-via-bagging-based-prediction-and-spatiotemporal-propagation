function [TPSPSAL_Img,TPSPSAL_RegionSal] = spatialPropagationNew12(TPSAL,CURINFOR,image,flow)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 仿照BSCA，迭代的更新策略
% 
% 2016.11.21 21:49PM
...
% 1.1 构建局传播阵 &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    ...
    FeaDist1      = FeaDist.*adjcMatrix;clear FeaDist
...
%     D_local       = diag(sum(optAff_local,2));
%     optAff_local_Norm = (D_local\eye(spNum))*optAff_local;
    
    C=a*normalizeSal(1./max(optAff_local'))+b;% 转置为寻找行最大值，即各样本i对应的权重值
    C_normal=diag(C);
...
for iter=1:iterNum
     fprintf('\n iter time %d ......',iter)
     PPSal=C_normal*PPSal+(1-C_normal).*diag(ones(1,supNum))*optAff_local_Norm*PPSal;

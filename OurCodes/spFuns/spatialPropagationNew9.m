function [TPSPSAL,TPSPSALRegionSal] = spatialPropagationNew9(CURINFOR,IMSAL_TPSAL1,param,cur_image,GPsign)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 在时域传播的基础上进行空域传播
% CURINFOR
...
% colDistM = (colDistM - minVal) / (maxVal - minVal + eps);
% W = exp(-colDistM * theta);
adjcMatrix_nn(adjcMatrix_nn>0)=1;
colDistM1 = colDistM.*adjcMatrix_nn;
D_sam     = mean(colDistM1,2);
D         = diag(D_sam);
colDistM1_norm  = inv(D)*colDistM1;
W = exp(-2*colDistM1_norm);
end

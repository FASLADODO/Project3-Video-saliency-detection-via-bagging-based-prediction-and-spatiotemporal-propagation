function [ TPSPSAL_Img,TPSPSAL_RegionSal ] = spfun19_8( TPSAL,CURINFOR,image,flow,param )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% GMR 图模型之图像四边不连接，background prior 连接； 前景传播
% MyManifoldRanking1
% 2016.12.08
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     regionFea0  = computeRegionFea(image,flow,tmpSPinfor);% 各区域的特征 L,a,b,man,ori
     ...
     regionFea = [tmpFEA.regionFea];% 20维的特征，这里仅选用L/a/b, Man/Ori 5维特征    
...
for iter=1:iterNum
    ...
    PPSal0 = PPSal > thresh;
    PPSal0 = double(PPSal0);% 前景
    
    % background priors
    bdIds = PPSal<(param.bdIds)*thresh;
    bdIds = find(bdIds==1);
    if isempty(bdIds)
        bdIds = tmpSPinfor.bdIds;
    end

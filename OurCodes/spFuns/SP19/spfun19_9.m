function [ TPSPSAL_Img,TPSPSAL_RegionSal ] = spfun19_9( TPSAL,CURINFOR,image,flow,param )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SOP， bdIds由 background priors代替， 图之四周边界不连接
% 
% 2016.12.08
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for iter=1:iterNum
    ...
    [PPSal1] = SOPFUN(adjcMatrix,bdIds,FeaDist1,PPSal,param);
    PPSal1 = normalizeSal(PPSal1);  
    
    % motion
    FeaDist2 = feaDistIntegrate(regionFea(:,8:9));
    [PPSal2] = SOPFUN(adjcMatrix,bdIds,FeaDist2,PPSal,param);

function UPDATA_DIC = updateDIC2(fpre_gt,CURINFOR,param,PRE_DIC)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 根据外观模型的结果，自适应的更新字典
% 衡量的是前后帧之间的相对变化
% 
...
% V6: 2016.08.31 20:14PM
% 于 updateDIC1 上进行修改，引入双帧字典构建模型: 即每次更新，
% 首先舍弃preDic中D0的pre部分；然后再结合当前的D0，构成新的D0
% 
...
%% 2. 更新
% 2.1 合并字典，
[newD0,indexs_pre] = mergePreCurD0(CURD0,PRE_DIC.D0);   
 
% 2.2 PCA
DB = D02DBNew(newD0,param);

% 2.3 训练
[UPDATA_DIC.beta, UPDATA_DIC.model, tmodel] = ...
       MultiFeaBoostingTrainNew1(DB,newD0,param);

% 2.4 舍弃predic之pre部分(indexs_pre.p  indexs_pre.n)
UPDATA_DIC.DB = DB; 
newD01 = abortPrepart(indexs_pre,newD0);
UPDATA_DIC.D0 = newD01;

clear newD0 DB CURD0 newD01 indexs_pre
...
% 每次合并时，均要记录下pre部分的位置标号！！！ 2016.08.31 20:22PM
function [result,indexs_pre] = mergePreCurD0(CURD0,PRED0)    
    ...
    index_pre_p = [(size(CURD0.P.colorHist_rgb,1)+1):size(result.P.colorHist_rgb,1)];   
    index_pre_n = [(size(CURD0.N.colorHist_rgb,1)+1):size(result.N.colorHist_rgb,1)];
    indexs_pre.p = index_pre_p;
    indexs_pre.n = index_pre_n;
    
    clear CURD0 PRED0 index_pre_p index_pre_n
end
% 01 舍弃pre部分
function newD0 = abortPrepart(indexs_pre,D0)
    index_pre_p = indexs_pre.p;
    index_pre_n = indexs_pre.n;
    
    D0.P.colorHist_rgb(index_pre_p,:) = [];
    D0.P.colorHist_lab(index_pre_p,:) = [];
    D0.P.colorHist_hsv(index_pre_p,:) = [];
    D0.P.lbpHist(index_pre_p,:)       = []; 
    D0.P.hogHist(index_pre_p,:)       = [];
    D0.P.regionCov(index_pre_p,:)     = [];    
    D0.P.geoDist(index_pre_p,:)       = [];     
    D0.P.flowHist(index_pre_p,:)      = [];      
    
    D0.N.colorHist_rgb(index_pre_n,:) = [];
    D0.N.colorHist_lab(index_pre_n,:) = [];
    D0.N.colorHist_hsv(index_pre_n,:) = [];
    D0.N.lbpHist(index_pre_n,:)       = [];    
    D0.N.hogHist(index_pre_n,:)       = [];  
    D0.N.regionCov(index_pre_n,:)     = [];    
    D0.N.geoDist(index_pre_n,:)       = [];     
    D0.N.flowHist(index_pre_n,:)      = [];

function UPDATA_DIC = updateDIC1(fpre_gt,CURINFOR,param,PRE_DIC)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 根据外观模型的结果，自适应的更新字典
% 衡量的是前后帧之间的相对变化
% 
...
if RATIO>=tl && RATIO<th % 部分更
    fprintf('\nPartialUPDATe = %d',RATIO)
    % 集中样本
    newD0 = mergePreCurD0(CURD0,PRE_DIC.D0);
    % PCA
    DB = D02DBNew(newD0,param);
    % save
    UPDATA_DIC.DB = DB; 
    UPDATA_DIC.D0 = newD0;
    clear newD0 DB CURD0
    
    [UPDATA_DIC.beta, UPDATA_DIC.model, tmodel] = ...
       MultiFeaBoostingTrainNew1(UPDATA_DIC.DB,UPDATA_DIC.D0,param);
end
% %% 2.2 计算区域显著性值 ---------------------
% spSal = computeGTinfor(fcur_gt,CURINFOR.spinfor);
% 

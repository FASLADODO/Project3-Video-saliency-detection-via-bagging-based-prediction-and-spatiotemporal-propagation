function UPDATA_DIC = ...
    updateDIC6_1(CURINFOR,param,fpre_GT,fpre_pre_GT,MVF_Foward_fn_f,MVF_Foward_fnn_f)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 字典更新，在updateDIC6的基础上进行修改，引入 pre & pre_pre 信息，
% 对objectIndex的物理位置进行限制，此举对于训练样本的获取非常关键！！！
% V1： 2016.09.03 23：14PM  updateDIC5
% V2:  2016.10.09 20:03PM  updateDIC6_1  
% 简单的取交集，确定种子点，然后确定物体大致范围以供预测，这样亦减少了参数阈值
...
% 暂时舍弃 fpre_pre_GT 2016.10.08 15:51PM
intersec = fpre_GT.* mapping_pre .* mapping_pre_pre;% .* fpre_pre_GT
union    = fpre_GT + mapping_pre +  mapping_pre_pre;% + fpre_pre_GT
union(union==4) = 1;
union(union==3) = 1;
union(union==2) = 1;

% newGT0 = intersec;
newGT0 = union;% 先并后交 2016.10.23 19:13PM
...
% end
% if iu_ratio>=0.5 %flow是可靠地  pre
%     newGT0 = intersec;disp(['intersec'])
% else
%     newGT0 = fpre_GT;disp(['pre'])
% end

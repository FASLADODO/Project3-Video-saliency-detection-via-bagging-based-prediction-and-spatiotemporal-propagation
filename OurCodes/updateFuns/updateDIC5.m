function UPDATA_DIC = ...
    updateDIC5(CURINFOR,param,fpre_GT,fpre_pre_GT,MVF_Foward_fn_f,MVF_Foward_fnn_f)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 字典更新，在updateDIC3的基础上进行修改，引入 pre & pre_pre 信息，
% 对objectIndex的物理位置进行限制，此举对于训练样本的获取非常关键！！！
% V1： 2016.09.03 23：14PM
% 
...
% revised in 2016.09.04 ---------------------------------------------------
    % pre_pre
    mapping_pre_pre = preGT_flow_mapping(fpre_pre_GT,MVF_Foward_fnn_f);
    intersec = fpre_GT.*mapping_pre_pre;
    union    = fpre_GT + mapping_pre_pre;
%     intersec = fpre_pre_GT.*mapping_pre_pre; % revised in 2016.10.08 20:45PM
%     union    = fpre_pre_GT + mapping_pre_pre;
    union(union==2) = 1;
    if sum(union(:))==0
    ...
    if iu_ratio>=param.flow_ratio(1) %flow是可靠地  pre
        newGT_pre_pre = intersec;disp(['intersec'])
    else
        newGT_pre_pre = fpre_pre_GT;disp(['pre_pre'])
    end
    ...
    if iu_ratio>=param.flow_ratio(2) %flow是可靠地  pre
        newGT_pre = intersec;disp(['intersec'])
    else
        newGT_pre = fpre_GT;disp(['pre'])
    end
    
    % intersect -------------------------
    newGT0 = newGT_pre_pre.*newGT_pre;
% -------------------------------------------------------------------------
...
% revised in 2016.09.03 23:28PM
eh = round(ee*(dh/2));
ew = round(ee*(dw/2));
% extend_length_h = round(ee*dh/2);
% eh = round(dh/2) + extend_length_h;
% extend_length_w = round(ee*dw/2);
% ew = round(dw/2) + extend_length_w;

function [spinforCur] = findTemporalAdjNew1(spinforPre, spinforCur,MVF_Backward,curORLabels,preORLabels)
...
%         findCorreSet(region_center_prediction,region_center_pre, tmp_curORLabels,tmp_preORLabels);
    ...
    spinforCur{ss,1}.mapsets = map_set_cur;% correSets correSets_dist
...
%     if 0 % 舍弃采用重叠度寻找相关集 2016.10.09 16:15PM
%     % 找出位于OR中的最佳匹配区域及相关集(最佳匹配点位于OR中) 
%     [sortValue, sortIndex] = sort(tmp_dist2);% 升序排列
%     for jj=1:length(sortIndex)
%         tmp_region_idx = sortIndex(jj);
%         idxSIGN        = ismember(tmp_region_idx,pre_out_OR_indexs);% OR外1 OR内0
%         if idxSIGN==0 % 位于OR内
...
%     end
%     
% %     tmp_region_idx = sortIndex(1);
%     correSetsLabel = adjmat_pre(tmp_region_idx1,:);% 找邻域
%     correSetsIndex = find(correSetsLabel==1);% 可能含有OR外区域
%     
%     % rvised in 2016.08.27 8:32AM 排除OR外区域，保留OR内区域
%     correSetSign = ismember(correSetsIndex, pre_out_OR_indexs);% OR外1 OR内0
%     correSetsIndex1 = correSetsIndex(correSetSign==0);% 保留OR中区域
%     correSets = [tmp_region_idx1(1),correSetsIndex1];
%     end
    
    % revised in 2016.10.09 16:15PM 基于前后帧重叠度的相关集寻找
    tmpoverlap = cur_pre_overlap(i,:);
    correSetsIndex = find(tmpoverlap>0);% 可能含有OR外区域
    ...
    ID = correSets(maxindex(1));clear correSets
    correSetsLabel = adjmat_pre(ID,:);
    correSetsIndex = find(correSetsLabel==1);
    correSetSign = ismember(correSetsIndex, pre_out_OR_indexs);
    correSetsIndex1 = correSetsIndex(correSetSign==0);
    ...
    % revised in 2016.10.13 晚
%     % 找出位于OR中的最佳匹配区域及相关集(最佳匹配点位于OR中且距离最小) 
%     [sortValue, sortIndex] = sort(tmp_dist2);% 升序排列
%     for jj=1:length(sortIndex)
%         tmp_region_idx = sortIndex(jj);
%         idxSIGN        = ismember(tmp_region_idx,pre_out_OR_indexs);% OR外1 OR内0

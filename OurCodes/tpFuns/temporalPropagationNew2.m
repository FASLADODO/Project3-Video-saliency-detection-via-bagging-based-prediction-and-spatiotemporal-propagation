...
%     fea{ss,1}.flowHist   
%     
% FullResultCurSal{ss,1}.FullValue 所有区域的显著性值
% FullResultCurSal{ss,1}.FullLabel 所有区域的标签（object/background）
% beta 分类器权重 & 对应标号
% SALS 时域传播后的各区域的显著性图
% 
% V2: 2016.08.18
...
%     tmpFullresult = normalizeSal(FullResultCurSal{ss,1});
    ...
    for sp=1:tmpSPcur.spNum
        ...
        if isempty(SIGNS)% 该区域位于OR外，等于其自身
...
%         tmpsalcur(sp,1) = delta + ratio*tmpFullresult(sp,1);
    ...
    % 计算映射传播的结果的compactness 2016.11.15 13:29PM

function [D0, beta, model, tmodel] = MultiFeaBoostingTrainNew2_1_0(DB,D0,ORLabels,spSal,param, spinfor)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for ss=1:length(ORLabels)
    ...
    tmpSPsal = spSal{ss,1};% 所有区域
    tmpSP = spinfor{ss,1};
    tmpPixellist = tmpSP.pixelList;
    
    ISORlabel = tmpORlabel(:,1);
    ISOBJlabel = tmpORlabel(:,3);
    % indexP indexN均是区域于整幅图像的index标记
    PNlabel = ISORlabel.*ISOBJlabel;
    index_P = find(PNlabel==1);% OR中前景标号: OR=1 OBJ=1
    index_N = [];% OR=1 OBJ=0 
    for ii=1:size(tmpORlabel,1)
       if ISORlabel(ii)==1 && ISOBJlabel(ii)==0 % OR中背景标号:OR=1 OBJ=0
           index_N = [index_N;ii];
       end
    end
    
    tmpPSAL = tmpSPsal(index_P,:);% 挑选正负样本对应的显著性值
    ...
    for pp=1:length(index_P)
        tmpPP = tmpPixellist{index_P(pp),1};
        numSPP = [numSPP;length(tmpPP)];
    end

    for nn=1:length(index_N)
        tmpNN = tmpPixellist{index_N(nn),1};
...
%     tmodel{i,1}.mapping.n = mapping_n;
    ...
    [mses,meanMSE,dec_v,pred_l,final_dic] = ...
        weakClassifierNew1_2(init_dic',d',sal, param, numPixel,spScaleLabel);% 1_1 新的字典学习方式 2016.10.28 8:07AM
    tmodel{i,1}.dic       = final_dic;
    tlabel{i}             = mses;% pred_l, 1*samplenum
    tdec{i}               = dec_v;% 1*samplenum
    tmeanmse{i}           = meanMSE;%1
    clear d l final_dic dec_v pred_l sal meanMSE mses
...
for t = 1:iter
    ...
    for j = 1:n_weaker
        if sum(j==tt) ~= 0 
            tbeta = [tbeta; -inf];
            error_weaker_marks = ...
                [error_weaker_marks,zeros(size(tlabel{j},1),1)];
            continue; 
        end
...
%         error_weaker0 = tlabel{j}' - tmeanmse{j};
%         error_weaker_mark = double(error_weaker0 >= 0);
%         error_weaker = sum(D{fi} .* error_weaker_mark);
%         error_weaker_marks = [error_weaker_marks,error_weaker_mark];
%         b = error_weaker;
    ...
    [var, idx] = min(tbeta);% 即错误率最小
    idx1 = find(tbeta == var);
    idx = idx1(end);
    if var>0.5 break; end % 限制错误率小于 0.5
        if var==0
            var = (1-var)/(var+eps);   
        else
            var = (1-var)/(var); 
...
%     model{t,1}.mapping = tmodel{idx,1}.mapping; % revised in 2016.10.13 8:11AM
    ...
    D{fi} = D{fi} .* ((1/var).^(1-error_weaker_marks(:,idx)));
    
%     l = eval(['l' num2str(fi)]);
%     l(l==0) = -1;
%     pred_l = (tlabel{fi})';% revised in 2016.10.10 19:46PM
%     pred_l(pred_l==0) = -1;
%     D{fi} = D{fi}.*exp(- var * (pred_l .* l));% 正分减小（同号），错分增大
